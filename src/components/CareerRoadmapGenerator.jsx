...
const CareerRoadmapGenerator = () => {
  ...
  useEffect(() => {
    const init = async () => {
      // Get user data from localStorage (support older keys too)
      ...
      // No user found: create a lightweight guest profile so the roadmap page still works.
      try {
        const tokenResponse = await fetch('/api/user/token', { method: 'POST' })
        const tokenData = await tokenResponse.json()
        const guestId = tokenData?.user?.id || crypto.randomUUID()
        ...
        setUser(guestUser)
        generateRoadmap(guestUser)
        setError('')
      } catch (err) {
        console.error('Unable to create guest user:', err)
